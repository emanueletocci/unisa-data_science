...
sqrt(x)
x^2 # or equivalently x*x, elementwise exponentiation
...
mystr=c('ciao', 'test'); mystr #creates a vector of 2 strings
strsplit("Try com mand", split = ' ')   #separates string elements based on spaces

################################################################################
# Indexing Data #
A=matrix(1:16,4,4)
A[2,3]                       # pick an element
A[1:3,2:4]                   # pick a submatrix
A [1:2,]                     # pick some rows and all columns
A[1,]
A[c(1,3),c(2,4)]             # pick specific elements of the matrix

test1 <- A[1,1:4,drop=FALSE] # drop=FALSE retains the matrix shape
dim(test1)
test2 <- A[1,1:4,drop=TRUE]  # drop=TRUE is the default
dim(test2)

A[-c(1,3),]                  # the minus means "except"                
dim(A)
...
table(myvec)  #counts the occurrences in myvec

v1=c(1:10) 
v2=c(11:20)
m1=cbind(v1,v2); m1    #creates a matrix (10 X 2)
m2=rbind(v1,v2); m2    #creates a matrix (2 X 10)
...
setwd('C:/Users/marco/Documents/RTDA/Didattica/2025_2026/Data Science/Lezioni/2025_21_10') #set the working directory
...
################################################################################
#### Create a Function #### 
myfun_sum=function(x,y) { # {} is the block specifying the function
  x+y
} 
myfun_sum(4,5)
...
tic() 
a<-numeric(10^7); for(i in 1:10^7) a[i]<-sin(2*pi*i/sqrt(2)) 
toc() # faster
...
cor(x,y)
plot(x,y)
lm(y~x)                       # this fits the linear model to the data
abline(lm(y~x),col='blue')    # plots a line
...
dev.print(device = pdf, "cont2.pdf")
fa=(f-t(f))/2
contour(x,y,fa,nlevels=15)
image(x,y,fa)
persp(x,y,fa)
persp(x,y,fa,theta=30, shade = 0.75, col = "gold", expand = 0.5, r = 2, ltheta = 25, ticktype = "detailed")
dev.print(device = jpeg, "persp.jpg", width=400,height=400)
dev.print(device = pdf, "persp.pdf", width=10,height=10)
...
dev.new()
par(mfrow=c(1,2))
contour(x,y,fbn,asp=1)
persp(x,y,fbn,theta = -30, phi = 25, 
      shade = 0.75, col = "gold", expand = 0.5, r = 2, 
      ltheta = 25, ticktype = "detailed",asp=1)
dev.print(device = jpeg, "mvn1.jpg", width=800,height=800)
dev.new()
par(mfrow=c(1,2))
contour(x,y,fbn2,asp=1)
persp(x,y,fbn2,theta = -30, phi = 25, 
      shade = 0.75, col = "gold", expand = 0.5, r = 2, 
      ltheta = 25, ticktype = "detailed",asp=1)
dev.print(device = jpeg, "mvn2.jpg", width=800,height=800)
dev.new()
par(mfrow=c(1,2))
contour(x,y,fbn3,asp=1)
persp(x,y,fbn3,theta = -30, phi = 25, 
      shade = 0.75, col = "gold", expand = 1.5, r = 2, 
      ltheta = 25, ticktype = "detailed",asp=1)
dev.print(device = jpeg, "mvn3.jpg", width=800,height=800)
dev.new()
par(mfrow=c(1,2))
contour(x,y,fbn4,asp=1)
persp(x,y,fbn4,theta = -30, phi = 25, 
      shade = 0.75, col = "gold", expand = 1.5, r = 2, 
      ltheta = 25, ticktype = "detailed",asp=1)
dev.print(device = jpeg, "mvn4.jpg", width=800,height=800)
par(mfrow=c(1,1))
...
plot(cylinders, mpg, col="red", varwidth=T, xlab="no. cylinders", ylab="MPG")
